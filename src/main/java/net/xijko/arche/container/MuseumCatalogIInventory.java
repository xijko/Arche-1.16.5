...
public class MuseumCatalogIInventory implements IInventory {
     ...
     */
    public static MuseumCatalogIInventory createForTileEntity(int size,
                                                          Predicate<PlayerEntity> canPlayerAccessInventoryLambda,
                                                          Notify markDirtyNotificationLambda) {
        return new MuseumCatalogIInventory(size, canPlayerAccessInventoryLambda, markDirtyNotificationLambda);
    }
     ...
     * @return the new ChestContents
     */
    public static MuseumCatalogIInventory createForClientSideContainer(int size) {
        return new MuseumCatalogIInventory(size);
    }
    ...
    public CompoundNBT serializeNBT()  {
        return calatogInputItem.serializeNBT();
    }
    ...
    public void deserializeNBT(CompoundNBT nbt)   {
        calatogInputItem.deserializeNBT(nbt);
    }
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack) {
        return calatogInputItem.isItemValid(index, stack);
    }
    ...
    public int getSizeInventory() {
        return calatogInputItem.getSlots();
    }

    @Override
    public boolean isEmpty() {
        for (int i = 0; i < calatogInputItem.getSlots(); ++i) {
            if (!calatogInputItem.getStackInSlot(i).isEmpty()) return false;
        }
        return true;
    }
    ...
    public ItemStack getStackInSlot(int index) {
        return calatogInputItem.getStackInSlot(index);
    }

    @Override
    public ItemStack decrStackSize(int index, int count) {
        if (count < 0) throw new IllegalArgumentException("count should be >= 0:" + count);
        return calatogInputItem.extractItem(index, count, false);
    }

    @Override
    public ItemStack removeStackFromSlot(int index) {
        int maxPossibleItemStackSize = calatogInputItem.getSlotLimit(index);
        return calatogInputItem.extractItem(index, maxPossibleItemStackSize, false);
    }

    @Override
    public void setInventorySlotContents(int index, ItemStack stack) {
        calatogInputItem.setStackInSlot(index, stack);
    }

    @Override
    public void clear() {
        for (int i = 0; i < calatogInputItem.getSlots(); ++i) {
            calatogInputItem.setStackInSlot(i, ItemStack.EMPTY);
        }
    }
    ...
    public ItemStack increaseStackSize(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = calatogInputItem.insertItem(index, itemStackToInsert, false);
    ...
    public boolean doesItemStackFit(int index, ItemStack itemStackToInsert) {
        ItemStack leftoverItemStack = calatogInputItem.insertItem(index, itemStackToInsert, true);
        return leftoverItemStack.isEmpty();
    }

    // ---------

    private MuseumCatalogIInventory(int size) {
        this.calatogInputItem = new ItemStackHandler(size);
    }

    private MuseumCatalogIInventory(int size, Predicate<PlayerEntity> canPlayerAccessInventoryLambda, Notify markDirtyNotificationLambda) {
        this.calatogInputItem = new ItemStackHandler(size);
        this.canPlayerAccessInventoryLambda = canPlayerAccessInventoryLambda;
        this.markDirtyNotificationLambda = markDirtyNotificationLambda;
    }
    ...
    private final ItemStackHandler calatogInputItem;
}
