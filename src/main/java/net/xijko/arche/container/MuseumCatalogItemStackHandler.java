...
public class MuseumCatalogItemStackHandler extends ItemStackHandler {
    public static final int MIN_SLOTS = 1;
    public static final int MAX_SLOTS = 36;

    public MuseumCatalogItemStackHandler(int numberOfSlots) {
        super(MathHelper.clamp(numberOfSlots, MIN_SLOTS, MAX_SLOTS));
        if (numberOfSlots < MIN_SLOTS || numberOfSlots > MAX_SLOTS) {
            throw new IllegalArgumentException("Invalid number of catalog slots:"+numberOfSlots);
        }
    }
    ...
    public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
        ...
        if (stack.isEmpty() || !(stack.getItem() instanceof ArcheArtifactItem)) return false;
