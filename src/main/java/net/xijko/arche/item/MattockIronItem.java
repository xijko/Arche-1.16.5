...
public class MattockIronItem extends ToolItem implements IVanishable {
    ...
    public MattockIronItem(float attackDamageIn, float attackSpeedIn, IItemTier tier, Properties builderIn) {
        super(attackDamageIn, attackSpeedIn, tier, EFFECTIVE_ON, builderIn.addToolType(ToolType.PICKAXE, tier.getHarvestLevel()));
    }
    ...
    public float getDestroySpeed(ItemStack stack, BlockState state) {
        ...
        if (state.isToolEffective(shovelType) || state.isToolEffective(pickType)) return this.efficiency;
        return 1.0F;
    }
